...
def read_bing_key():
    ...
    bing_api_key = None
    try:
        with open(os.path.join(settings.BASE_DIR,'bing.key'), 'r') as f:
...
def run_query(search_terms):
    ...
    query = parse.quote(query) # Py3

    # Construct the latter part of our request's URL.
    # Sets the format of the response to JSON and sets other properties.

    search_url = "{0}{1}?$format=json&$top={2}&$skip={3}&Query={4}".format(root_url, service, results_per_page, offset,
                                                                           query)
...
def main():
    run_query('Hello')#insert your code here
